...
#' k <- lapply(X = ct_files, FUN = extract_eligibility)
#' k <- dplyr::bind_rows(k)
#' View(k)
#'
extract_eligibility <- function(trial_path){
  ...
  tryCatch(expr = {
    eligibility_textblock <- xpathSApply(doc = data_root, path = "//criteria/textblock", fun = xmlValue)
  },
  ...
  eligibility_inc <- trimws(gsub(pattern = "\\s+", replacement = " ", x = eligibility_inc))
  ...
  eligibility_exc <- trimws(gsub(pattern = "\\s+", replacement = " ", x = eligibility_exc))
  ...
  eligibility_criteria <- data.frame("x" =
    c(
    gender <- paste("Gender:", xpathSApply(doc = data_root, path = "//eligibility/gender", fun = xmlValue)),
    minimum_age <- paste("Minimum_Age:", xpathSApply(doc = data_root, path = "//eligibility/minimum_age", fun = xmlValue)),
    maximum_age <- paste("Maximum_Age:", xpathSApply(doc = data_root, path = "//eligibility/maximum_age", fun = xmlValue)),
    healthy_volunteers <- paste("Healthy_Volunteers:", xpathSApply(doc = data_root, path = "//eligibility/healthy_volunteers", fun = xmlValue))
    ), "Type" = "Eligibility"
  )
